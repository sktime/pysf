...
if False:
    ...
    if False:
        ...
        # TODO: update after testing: I've isolated a single "curve" for prediction
        # Get training & test splits
        #splits = list(data_growth.generate_series_folds(series_splitter=KFold(n_splits=93)))
        ...
        for (training_instance, validation_instance) in data_growth.generate_series_folds(series_splitter=KFold(n_splits=5)):
        
            training_instance.visualise(title='Training instance', filter_value_colnames='height')
            validation_instance.visualise(title='Validation instance', filter_value_colnames='height')
            
            include_timestamps_as_features = False
            times = np.arange(14,18.5,0.5)
            predictor = SingleCurveTabularWindowedPredictor.initWithAllParams(classic_estimator=LinearRegression(), allow_missing_values=False, width_input=3, width_output=2)
            print(predictor)
            
            predictor.fit(X=training_instance, prediction_times=times, input_non_time_features=['boy', 'girl'], prediction_features='height') 
            scoring_results = predictor.score(X=validation_instance, prediction_times=times, input_non_time_features=['boy', 'girl'], prediction_features='height') 
    
            individual_result = scoring_results['height']
            individual_result.Y_true.visualise(filter_value_colnames='height', title='Y_true')
            individual_result.Y_hat.visualise(filter_value_colnames='height', title='Y_hat')
            individual_result.residuals.visualise()
            individual_result.err.visualise_per_timestamp()
            #print(Y_hat._data_vs_times_df)
    ...
    if False:
        ...
        # TODO: update after testing: I've isolated a single "curve" for prediction
        # Get training & test splits
        splits = list(data_weather.generate_series_folds(series_splitter=KFold(n_splits=35)))
        #splits = list(data_weather.generate_series_folds(series_splitter=KFold(n_splits=35)))
        (training_instance, validation_instance) = splits[0]
        ...
        predictor.fit(X=training_instance, input_time_feature=include_timestamps_as_features, prediction_times=times, prediction_features=['tempav','precav'])
        scoring_results = predictor.score(X=validation_instance, input_time_feature=include_timestamps_as_features, prediction_times=times, prediction_features=['tempav','precav'])

        individual_result = scoring_results['tempav']
        individual_result.Y_true.visualise(filter_value_colnames='tempav', title='Y_true')
        individual_result.Y_hat.visualise(filter_value_colnames='tempav', title='Y_hat')
        individual_result.residuals.visualise()
        individual_result.err.visualise_per_timestamp()
        
        individual_result = scoring_results['precav']
        individual_result.Y_true.visualise(filter_value_colnames='precav', title='Y_true')
        individual_result.Y_hat.visualise(filter_value_colnames='precav', title='Y_hat')
        individual_result.residuals.visualise()
        individual_result.err.visualise_per_timestamp()
    ...
    if False:
        ...
        include_timestamps_as_features = False
        times = np.arange(14,18.5,0.5)
        predictor = MultiCurveTabularWindowedPredictor.initWithAllParams(classic_estimator=LinearRegression(), allow_missing_values=False, width_input=3, width_output=2)
        print(predictor)
