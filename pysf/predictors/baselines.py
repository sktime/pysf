...
if False:
    ...
    estimator = LinearInterpolator()
    
    # SKlearn requires 2D arrays as input. I've converted the X arrays here to be in the shape of a single feature:
    fit_X = np.arange(5, 15).reshape(-1,1)
    fit_y = np.random.rand(fit_X.shape[0], 3)
    predict_X = np.array([1, 2, 4, 5, 6.2, 6.8, 13.5, 14.5, 15, 16, 20])  .reshape(-1,1)
    
    print(fit_X)
    print(fit_y)
    print(predict_X)
    
    estimator.fit(X=fit_X, y=fit_y)
    #estimator.predict(X=predict_X)
    estimator.predict_and_visualise(X=predict_X)
...
if False:
    ...
    generalisation_metrics_overall_df = evaluator_weather.evaluate(chart_intermediate_results=True)
    
    # Chart overall results
    evaluator_weather.chart_overall_performance(feature_name='tempav', metric='rmse', best_n_results=20, figsize=(5, 2))
    evaluator_weather.chart_overall_performance(feature_name='precav', metric='rmse', best_n_results=20, figsize=(5, 2))
